		...
		if (playerMagicData.getAdditionalCastData() instanceof TargetEntityCastData targetingData) {
			...
			if (targetEntity != null) {
				...
				for (int i = 0; i < getDuration(spellLevel, entity); i++) {
					...
					Void_Rune_Entity voidRune = new Void_Rune_Entity(level,
							targetX + Mth.cos(f1) * 1.5D, blockPos.getY() + d0,
							targetZ + Mth.sin(f1) * 1.5D, (float) d0, delay,
							getDamage(spellLevel), entity);
					level.addFreshEntity(voidRune);
					targetEntity.addEffect(new MobEffectInstance(
							CSPotionEffectRegistry.SUMMON_VOID_RUNE.get(),
							getEffectDuration(spellLevel, entity), spellLevel - 1, false,
							false, false));
				}
			}
		}
