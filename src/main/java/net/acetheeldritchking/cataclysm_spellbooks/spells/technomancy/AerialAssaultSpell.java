...
public class AerialAssaultSpell extends AbstractHarbingerSpell {
	...
	public void onCast(Level level, int spellLevel, LivingEntity entity, CastSource castSource,
		...
		if (!(playerMagicData.getAdditionalCastData() instanceof TargetAreaCastData)) {
			Vec3 targetArea = Utils.moveToRelativeGroundLevel(level,
					Utils.raycastForEntity(level, entity, 40, true).getLocation(), 12);
			playerMagicData.setAdditionalCastData(
					new TargetAreaCastData(targetArea, TargetedAreaEntity.createTargetAreaEntity(
							level, targetArea, getRadius(spellLevel, entity), 0xfa1033)));
		}
	...
	public void onServerCastTick(Level level, int spellLevel, LivingEntity entity,
		...
		if (playerMagicData != null && (playerMagicData.getCastDurationRemaining() + 1) % 4 == 0) {
			if (playerMagicData.getAdditionalCastData() instanceof TargetAreaCastData targetAreaCastData) {
				...
				Vec3 spawn = center
						.add(new Vec3(0, entity.getY(), entity.getRandom().nextFloat() * radius)
	...
	public void shootMissiles(Level level, LivingEntity caster, int spellLevel, Vec3 spawn) {
		...
		Wither_Missile_Entity missile = new Wither_Missile_Entity(ModEntities.WITHER_MISSILE.get(), caster,
				spawn.add(offset).x, spawn.add(offset).y, spawn.add(offset).z, new Vec3(0, d2, 0),
				getDamage(spellLevel, caster), level);
