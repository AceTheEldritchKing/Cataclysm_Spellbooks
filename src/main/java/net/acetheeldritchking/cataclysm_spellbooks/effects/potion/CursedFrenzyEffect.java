...
public class CursedFrenzyEffect extends MobEffect {
	...
	public void applyEffectTick(LivingEntity pLivingEntity, int pAmplifier) {
		List<Entity> entitiesNearby = pLivingEntity.level().getEntities(pLivingEntity,
				pLivingEntity.getBoundingBox().inflate(0.25, 0.5, 0.25));

		if (!entitiesNearby.isEmpty()) {
			for (Entity targets : entitiesNearby) {
				if (targets instanceof LivingEntity) {
					DamageSources.applyDamage(targets, pAmplifier, SpellRegistries.CURSED_RUSH.get()
							.getDamageSource(pLivingEntity));
		...
		if (pLivingEntity.level().isClientSide) {
			...
			if (pLivingEntity.tickCount % 3 == 0) {
				...
				pLivingEntity.level().addParticle(new RingParticleOptions(yaw, pitch, 40, 86, 236, 204,
						1.0f, 50f, false,
						RingBehaviourHelper.convert(
								RingParticle.EnumRingBehavior.GROW_THEN_SHRINK)),
						x, y, z, 0, 0, 0);
				pLivingEntity.level().addParticle(new RingParticleOptions(yaw2, pitch, 40, 86, 236, 204,
						1.0f, 50f, false,
						RingBehaviourHelper.convert(
								RingParticle.EnumRingBehavior.GROW_THEN_SHRINK)),
						x, y, z, 0, 0, 0);
