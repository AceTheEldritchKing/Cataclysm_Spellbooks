...
public class MoltenBulletProjectile extends AbstractMagicProjectile implements GeoEntity {
	...
	public void trailParticles() {
		for (int i = 0; i < 5; i++) {
			...
			this.level().addParticle(ParticleTypes.FLAME, x, y, z, -getDeltaMovement().x,
					-getDeltaMovement().y, -getDeltaMovement().z);
		}
	}

	@Override
	public void impactParticles(double x, double y, double z) {
		MagicManager.spawnParticles(this.level(), ModParticle.FLARE_EXPLODE.get(), x, y, z, 10, .1, .1, .1, .18,
				true);
	}
	...
	public void tick() {
		...
		if (this.level().isClientSide) {
			...
			this.level().addParticle(new LightTrailParticleOptions(r, g, b, 0.1F, this.getBbHeight() / 2,
					this.getId()), x, y, z, 0, 0, 0);
		}
	...
	protected void onHitEntity(EntityHitResult pResult) {
		...
		if (!target.hurt(SpellRegistries.SCORCHED_EARTH.get().getDamageSource(this, getOwner()),
				this.getDamage())) {
		...
		if (!this.level().isClientSide) {
			...
			for (Entity entity : entities) {
				...
				if (distanceToSqr < radiusSqr && canHitEntity(entity) && Utils.hasLineOfSight(
					...
					ScreenShake_Entity.ScreenShake(this.level(), entity.position(), 2.0F, 0.15F, 20,
							20);

					DamageSources.applyDamage(entity, damage, SpellRegistries.SCORCHED_EARTH.get()
	...
	public void spawnXFlameJet(int rune, double time) {
		for (int i = 0; i < 4; i++) {
			...
			for (int k = 0; k < rune; ++k) {
				double d2 = 0.8 * (k + 1);
				int d3 = (int) (time * (k + 1));
				CSUtils.spawnFlameJets(this.level(),
						this.getX() + (double) Mth.cos(throwAngle) * 1.25 * d2,
						this.getZ() + (double) Mth.sin(throwAngle) * 1.25 * d2,
						this.getY() - 2.0, this.getY() + 2.0, throwAngle, d3,
						(LivingEntity) getOwner(), this.getDamage());
			}
		}
	}
