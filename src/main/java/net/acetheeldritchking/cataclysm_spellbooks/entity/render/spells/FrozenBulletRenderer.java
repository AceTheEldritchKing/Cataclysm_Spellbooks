...
public class FrozenBulletRenderer extends GeoProjectilesRenderer<FrozenBulletProjectile> {
    public FrozenBulletRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new FrozenBulletModel());
    }

    @Override
    public void render(FrozenBulletProjectile projectile, float yaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        poseStack.pushPose();
        ...
        AnimationEvent<FrozenBulletProjectile> predicate = new AnimationEvent<>(projectile, 0, 0, partialTick, false, Collections.singletonList(new EntityModelData()));
