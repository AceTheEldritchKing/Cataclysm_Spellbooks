...
public class GravitationPullSpell extends AbstractSpell {
	...
	public List<MutableComponent> getUniqueInfo(int spellLevel, LivingEntity caster) {
		return List.of(
				...
				Component.translatable("ui.cataclysm_spellbooks.difference",
						getDifference(spellLevel, caster)));
	}
	...
	public void onCast(Level level, int spellLevel, LivingEntity entity, CastSource castSource,
		...
		List<LivingEntity> entitiesNearby = level.getEntitiesOfClass(LivingEntity.class,
		...
		entitiesNearby.forEach(target -> {
			...
			if (entity.isCrouching()) {
				// Pull entities away
				target.setDeltaMovement(difference.multiply(-getDifference(spellLevel, entity),
						-getDifference(spellLevel, entity), -getDifference(spellLevel, entity))
						.normalize());
			} else {
				// Drag entities in
				...
				if (distance <= 1) {
					// entities can't really exist in the same position as the caster (they just
					// bounce around)
					return;
				}
				...
				if (overshot > 1) {
					// max. pull should be the distance to the entity, not further (reduces bouncing
					// around)
					difference = new Vec3(difference.x() / overshot, difference.y() / overshot,
							difference.z() / overshot);
				}

				// if the added movement is too much they fly past the target - 0.2F
				difference = difference.multiply(getDifference(spellLevel, entity),
						getDifference(spellLevel, entity), getDifference(spellLevel, entity))
						.normalize();
		...
		for (int i = 0; i < count; i++) {
			...
			MagicManager.spawnParticles(entity.level(), ParticleTypes.DRAGON_BREATH,
					entity.position().x + x, entity.position().y, entity.position().z + z, 1, 0, 0,
					0, 0.1, false);
		}
