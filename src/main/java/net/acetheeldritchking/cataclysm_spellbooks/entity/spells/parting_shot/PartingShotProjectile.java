...
public class PartingShotProjectile extends AbstractMagicProjectile implements GeoEntity {
	...
	public void trailParticles() {
		...
		this.level().addParticle(new TrackLightningParticleOptions(232, 59, 59), vec3.x, vec3.y, vec3.z, vec3.x,
				vec3.y, vec3.z);
	}
	...
	public void impactParticles(double x, double y, double z) {
		...
		// (level, new TrackLightningParticle.OrbData(255, 6, 62), x, y, z, 3, 0, 0, 0,
		// 0.5, true);
	}
	...
	public void tick() {
		...
		arcVec = center.add(new Vec3((this.random.nextFloat() - 0.5F), (this.random.nextFloat() - 0.5F),
				(this.random.nextFloat() - 0.5F)));

		// this.level.addParticle(new TrackLightningParticle.OrbData(255, 6, 62),
		// arcVec.x, arcVec.y, arcVec.z, 0, 0, 0);

		this.level().addParticle(new TrackLightningParticleOptions(232, 59, 59), center.x, center.y, center.z,
				arcVec.x, arcVec.y, arcVec.z);
	...
	protected void onHitEntity(EntityHitResult pResult) {
		...
		if (target instanceof LivingEntity livingTarget) {
			...
			livingTarget.addEffect(new MobEffectInstance(CSPotionEffectRegistry.DISABLED_EFFECT.get(), 100,
					0, true, true, true));
		}

		if (!target.hurt(SpellRegistries.PARTING_SHOT.get().getDamageSource(this, getOwner()),
				this.getDamage())) {
		...
		if (!this.level().isClientSide) {
			...
			if (CSConfig.doSpellGriefing.get()) {
				...
				if (this.level().isEmptyBlock(pos)) {
					...
					|| ForgeEventFactory.getMobGriefingEvent(this.level(), entity)) {
				pos = pResult.getBlockPos().relative(pResult.getDirection());
				if (this.level().isEmptyBlock(pos)) {
					this.level().setBlockAndUpdate(pos, BaseFireBlock.getState(this.level(), pos));
				}
	...
	protected void onHit(HitResult hitresult) {
		...
		if (!this.level().isClientSide) {
			...
			Vec3 losPoint = Utils.raycastForBlock(this.level(), this.position(),
					this.position().add(0, 2, 0), ClipContext.Fluid.NONE).getLocation();

			for (Entity entity : entities) {
				double distanceToSqr = entity.distanceToSqr(hitresult.getLocation());

				if (distanceToSqr < radiusSqr && canHitEntity(entity) && Utils.hasLineOfSight(
						this.level(), losPoint, entity.getBoundingBox().getCenter(), true)) {
					double modifier = (1 - distanceToSqr / radiusSqr);
					float damage = (float) (getDamage() * modifier);

					ScreenShake_Entity.ScreenShake(this.level(), entity.position(), 5.0F, 0.15F, 20,
							20);

					DamageSources.applyDamage(entity, damage, SpellRegistries.PARTING_SHOT.get()
			...
			if (CSConfig.doSpellGriefing.get()) {
				// EXPLOSION
				Explosion explosion = new Explosion(this.level(), null,
						SpellRegistries.PARTING_SHOT.get().getDamageSource(this, getOwner()),
						null, this.getX(), this.getY(), this.getZ(),
						this.getExplosionRadius() / 2, true,
						Explosion.BlockInteraction.DESTROY);
				if (!net.minecraftforge.event.ForgeEventFactory.onExplosionStart(this.level(),
						explosion)) {
					explosion.explode();
			...
			MagicManager.spawnParticles(this.level(),
					new BlastwaveParticleOptions(SchoolRegistry.FIRE.get().getTargetingColor(),
