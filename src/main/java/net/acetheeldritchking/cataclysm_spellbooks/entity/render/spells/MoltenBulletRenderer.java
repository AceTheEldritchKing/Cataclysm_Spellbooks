...
public class MoltenBulletRenderer extends GeoProjectilesRenderer<MoltenBulletProjectile> {
    public MoltenBulletRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new MoltenBulletModel());
    }

    @Override
    public void render(MoltenBulletProjectile projectile, float yaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        poseStack.pushPose();
        ...
        AnimationEvent<MoltenBulletProjectile> predicate = new AnimationEvent<>(projectile, 0, 0, partialTick, false, Collections.singletonList(new EntityModelData()));
