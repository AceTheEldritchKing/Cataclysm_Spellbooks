...
public class BloodCrystalRenderer extends GeoProjectilesRenderer<BloodCrystalProjectile> {
    public BloodCrystalRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new BloodCrystalModel());
    }

    @Override
    public void render(BloodCrystalProjectile projectile, float yaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        poseStack.pushPose();
        ...
        AnimationEvent<BloodCrystalProjectile> predicate = new AnimationEvent<>(projectile, 0, 0, partialTick, false, Collections.singletonList(new EntityModelData()));
